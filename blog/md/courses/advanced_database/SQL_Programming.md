...
Every thing here in this class should be typed in the terminal to get yourself familiarized with the sql commands.

## Outline

* SQL Refresh
* A in-class lab to build a small banking system
* Python and JDBC Programming with MySQL

# Part I SQL Programming Refresh

## Warm up

Commands to try, open a dos, shell terminals and type the following.

>In windows, you might be able to use `MySQL Shell`, it would log you in automatically.
...
SHOW DATABASES where LENGTH(`Database`) > 4;
```
...
> you could also set it on column level character set.
 For instance, `column_name varchar(20) character set utf8.`
...
```
>Hint: please pay attention to the above `create`, where is the constraint?

Next, let us create another table,
...
Check the new table by doing the following,

`DESC favorite_food;`

> Hint: How did we previously check the table in cmd?
...
> Hint: check more here, https://learn.microsoft.com/en-us/sql/relational-databases/xml/use-auto-mode-with-for-xml?view=sql-server-ver16
...
# Part II SQL Practice- A small banking system  

* SQL Scripts used for Homework
* The following is a small banking system. Please run them and create the schemas in your db.
* And try to analyze the correlations with the tables. And think about how to visualize the relations.

> Note: The following scripts are used to program with Python and Java in your second homework.
...
# Part III JDBC Programming

## Takeaway

* JDBC is like ODBC. It is as driver.
* In Java world, it is a jar.
* Tomcat is also a jar, Kotlin language are jars and many things are jars... and they are actually a cooning surface libraries on top of JVM which is java.exe -jar some_name.jar.
* More advanced Java Database connection, we will use SpringBoot framework, but it is not mentioned in this scope of study.
...
## Python MySQL Programming Reference
